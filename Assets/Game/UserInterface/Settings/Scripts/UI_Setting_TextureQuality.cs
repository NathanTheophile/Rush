using UnityEngine;
using UnityEngine.UI;

namespace Rush.UI
{
    public class UI_Setting_TextureQuality : MonoBehaviour
    {
        #region _____________________________/ VALUES

        [SerializeField] private Dropdown _Dropdown;

        #endregion

        #region _____________________________| UNITY

        private void Awake() => _Dropdown ??= GetComponentInChildren<Dropdown>();

        private void OnEnable()
        {
            if (_Dropdown != null)
                _Dropdown.value = QualitySettings.globalTextureMipmapLimit;
        }

        private void Start()
        {
            if (_Dropdown == null)
                return;

            _Dropdown.onValueChanged.AddListener(Apply);
            Apply(_Dropdown.value);
        }

        #endregion

        #region _____________________________| METHODS

        private void Apply(int pIndex)
        {
            pIndex = Mathf.Max(0, pIndex);
            QualitySettings.globalTextureMipmapLimit = pIndex;
        }

        #endregion
    }
}